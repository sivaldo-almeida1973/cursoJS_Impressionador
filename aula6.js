const numero1 = 27;
const numero2 = 23.25;
const numero3 = -40;
const total = numero1 + numero2 + numero3;

console.log(total);




const texto1 = 'Aqui temos uma string normal.';
const texto2 = 'Já essa, apesar das aspas duplas ao invés das simples, também é uma string normal';

const numeroQueQueroRegistrar = 31472;
const stringTemplate = `O número que decidi registrar foi o ${numeroQueQueroRegistrar}`;



console.log(stringTemplate) //fará aparecer na tela o seguinte te